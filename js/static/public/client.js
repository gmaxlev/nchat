...
function scaffoldIterator(structure, handler) {
  ...
  for (const serviceName of services) {
    ...
    for (const methodName of methods) {
      api[serviceName][methodName] = handler(
        serviceName,
        methodName,
        structure[serviceName][methodName]
      );
    }
  }
}
...
function scaffold(url, structure) {
  const transport = url.startsWith("ws") ? scaffoldWs : scaffoldHttp;
  return transport(url, structure);
}

(async () => {
  window.api = await scaffold("ws://127.0.0.1:8001", {
    user: {
      create: ["record"],
      read: ["id"],
      update: ["id", "record"],
